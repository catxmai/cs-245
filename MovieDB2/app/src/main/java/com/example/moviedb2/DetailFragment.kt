...
class DetailFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        release_text.text = movie.release_date
        rating_text.text = "Rating: "+movie.vote_average.toString()
        overview_text.text = movie.overview
        ratingBar.rating = movie.rating.toFloat()
        PosterLoader.getInstance().loadURL(movie.backdrop_path, poster_img)

        val current = viewModel.currentMovie.value!!
        ratingBar.setOnRatingBarChangeListener { ratingBar, rating, fromUser ->
            current.rating = rating
            viewModel.saveCurrentMovie()
            viewModel.updateMovieByID()
